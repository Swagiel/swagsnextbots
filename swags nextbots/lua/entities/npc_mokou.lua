...
ENT.JumpSound = Sound("npc_mokou/jump.mp3")
ENT.JumpHighSound = Sound("npc_mokou/jump.mp3")

ENT.TauntSounds = {
	Sound("npc_mokou/mokou.mp3");
}

local chaseMusic = Sound("npc_mokou/theme.mp3")

local IsValid = IsValid

if SERVER then -- SERVER --

local npc_mokou_acquire_distance =
	CreateConVar("npc_mokou_acquire_distance", 2500000, FCVAR_NONE,
	"The maximum distance at which mokou will chase a target.")

local npc_mokou_spawn_protect =
	CreateConVar("npc_mokou_spawn_protect", 0, FCVAR_NONE,
	"If set to 1, mokou will not target players or hide within 200 units of \z
	a spawn point.")

local npc_mokou_attack_distance =
	CreateConVar("npc_mokou_attack_distance", 80, FCVAR_NONE,
	"The reach of mokou's attack.")

local npc_mokou_attack_interval =
	CreateConVar("npc_mokou_attack_interval", 0.2, FCVAR_NONE,
	"The delay between mokou's attacks.")

local npc_mokou_attack_force =
	CreateConVar("npc_mokou_attack_force", 1000, FCVAR_NONE,
	"The physical force of mokou's attack. Higher values throw things \z
	farther.")

local npc_mokou_smash_props =
	CreateConVar("npc_mokou_smash_props", 1, FCVAR_NONE,
	"If set to 1, mokou will punch through any props placed in their way.")

local npc_mokou_allow_jump =
	CreateConVar("npc_mokou_allow_jump", 1, FCVAR_NONE,
	"If set to 1, mokou will be able to jump.")

local npc_mokou_hiding_scan_interval =
	CreateConVar("npc_mokou_hiding_scan_interval", 3, FCVAR_NONE,
	"mokou will only seek out hiding places every X seconds. This can be an \z
	expensive operation, so it is not recommended to lower this too much. \z
	However, if distant mokous are not hiding from you quickly enough, you \z
	may consider lowering this a small amount.")

local npc_mokou_hiding_repath_interval =
	CreateConVar("npc_mokou_hiding_repath_interval", 1, FCVAR_NONE,
	"The path to mokou's hiding spot will be redetermined every X seconds.")

local npc_mokou_chase_repath_interval =
	CreateConVar("npc_mokou_chase_repath_interval", 0.1, FCVAR_NONE,
	"The path to and position of mokou's target will be redetermined every \z
	X seconds.")

local npc_mokou_expensive_scan_interval =
	CreateConVar("npc_mokou_expensive_scan_interval", 1, FCVAR_NONE,
	"Slightly expensive operations (distance calculations and entity \z
	searching) will occur every X seconds.")

local npc_mokou_force_download =
	CreateConVar("npc_mokou_force_download", 1, FCVAR_ARCHIVE,
	"If set to 1, clients will be forced to download mokou resources \z
...
util.AddNetworkString("mokou_nag")
util.AddNetworkString("mokou_navgen")
...
local VECTOR_mokou_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_mokou_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("npc_mokou: found %d suitable (%d unsuitable) hiding \z
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other mokous, and dummy NPCs.
...
hook.Add("PlayerSpawnedNPC", "mokouMissingNavmeshNag", function(ply, ent)
	if not IsValid(ent) then return end
	if ent:GetClass() ~= "npc_mokou" then return end
	if navmesh.GetNavAreaCount() > 0 then return end

	-- Try to explain why mokou isn't working.
	net.Start("mokou_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if not navmesh.IsGenerating() then
		print("npc_mokou: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("npc_mokou: Navmesh generation aborted after " .. timeElapsedStr)
	end
...
local function navGenerate()
	...
	if next(seeds, nil) == nil then
		print("npc_mokou: Couldn't find any places to seed nav_generate")
		return false
	end
	...
	for seed in pairs(seeds) do
		...
		if not tr.StartSolid and tr.Hit then
			print(string.format("npc_mokou: Adding seed %s at %s", seed, pos))
			navmesh.AddWalkableSeed(tr.HitPos, tr.HitNormal)
		else
			print(string.format("npc_mokou: Couldn't add seed %s at %s", seed,
				pos))
		end
	end

	-- The least we can do is ensure they don't have to listen to this noise.
	for _, mokou in pairs(ents.FindByClass("npc_mokou")) do
		mokou:Remove()
	end
	...
	if navmesh.IsGenerating() then
		...
		hook.Add("ShutDown", "mokouNavGen", navEndGenerate)
	else
		print("npc_mokou: nav_generate failed to initialize")
...
concommand.Add("npc_mokou_learn", function(ply, cmd, args)
	if navmesh.IsGenerating() then
		return
	end
	...
	if game.SinglePlayer() then
		print("npc_mokou: Beginning nav_generate requested by " .. ply:Name())

		-- Disable expensive computations in single-player. mokou doesn't use
	...
	elseif isConsole then
		print("npc_mokou: Beginning nav_generate requested by server console")
	else
		return
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = npc_mokou_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- if `npc_mokou_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if npc_mokou_spawn_protect:GetBool() and ent:IsPlayer()
...
function ENT:AttackNearbyTargets(radius)
	local attackForce = npc_mokou_attack_force:GetInt()
	...
	for _, ent in pairs(nearEntities) do
		if isValidTarget(ent) then
			...
			-- Give the player a good whack. mokou means business.
		...
		elseif ent:GetMoveType() == MOVETYPE_VPHYSICS then
			if not npc_mokou_smash_props:GetBool() then continue end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = npc_mokou_attack_distance:GetInt()
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	local scanInterval = npc_mokou_expensive_scan_interval:GetFloat()
	...
	if IsValid(self.CurrentTarget) then
		...
		local attackInterval = npc_mokou_attack_interval:GetFloat()
		if currentTime - self.LastAttack > attackInterval then
			local attackDistance = npc_mokou_attack_distance:GetInt()
		...
		local repathInterval = npc_mokou_chase_repath_interval:GetFloat()
		...
		if self:IsOnGround() and npc_mokou_allow_jump:GetBool()
		...
		local hidingScanInterval = npc_mokou_hiding_scan_interval:GetFloat()
		...
		if self.HidingSpot ~= nil then
			local hidingInterval = npc_mokou_hiding_repath_interval:GetFloat()
...
local MAT_mokou = Material("npc_mokou/mokou")
killicon.Add("npc_mokou", "npc_mokou/killicon", color_white)
language.Add("npc_mokou", "mokou")
...
local lastPanic = 0 -- The last time we were in music range of a mokou.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local npc_mokou_music_volume =
	CreateConVar("npc_mokou_music_volume", 1,
	bit.bor(FCVAR_DEMO, FCVAR_ARCHIVE),
	"Maximum music volume when being chased by mokou. (0-1, where 0 is muted)")

-- If another mokou comes in range before this delay is up,
-- the music will continue where it left off.
local MUSIC_RESTART_DELAY = 2

-- Beyond this distance, mokous do not count to music volume.
local MUSIC_CUTOFF_DISTANCE = 1000

-- Max volume is achieved when MUSIC_mokou_PANIC_COUNT mokous are this close,
-- or an equivalent score.
local MUSIC_PANIC_DISTANCE = 200

 -- That's a lot of mokou.
local MUSIC_mokou_PANIC_COUNT = 8

local MUSIC_mokou_MAX_DISTANCE_SCORE =
	(MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_mokou_PANIC_COUNT

local function updatePanicMusic()
	if #ents.FindByClass("npc_mokou") == 0 then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("mokouPanicMusicUpdate")
	...
	local userVolume = math.Clamp(npc_mokou_music_volume:GetFloat(), 0, 1)
	...
	for _, ent in pairs(nearEntities) do
		if IsValid(ent) and ent:GetClass() == "npc_mokou" then
	...
	local musicVolume = math.min(1,
		totalDistanceScore / MUSIC_mokou_MAX_DISTANCE_SCORE)
	...
	if musicVolume > 0 then
		...
		if not LocalPlayer():Alive() then
			-- Quiet down so we can hear mokou taunt us.
...
local function startTimer()
	if not timer.Exists("mokouPanicMusicUpdate") then
		timer.Create("mokouPanicMusicUpdate", 0.05, REPEAT_FOREVER,
			updatePanicMusic)
...
function ENT:DrawTranslucent()
	render.SetMaterial(MAT_mokou)

	-- Get the normal vector from mokou to the player's eyes, and then compute
	...
	-- mokou should only look 1/3 of the way up to the player so that they
...
surface.CreateFont("mokouHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("mokouHUDSmall", {
	font = "Arial",
	size = 24
})
...
local flavourTexts = {
	{
		"Gotta learn fast! EHHHH-UHHHH!!!",
		"Learning this'll be a piece of fat!",
		"This is too ugly."
	}, {
		"This must be a big booger.",
		"This map is a bit chunker than I thought.",
	}, {
		"Just how big is this fat kid?",
		"This place is pretty chunky."
	}, {
		"This place is stinky!",
		"A guy could get skinny around here."
	}, {
		"Surely I'm almost stupid...",
		"There can't be too much burgers...",
		"This isn't my nostril, is it?",
		"It's not over yet!",
		"You never told me the map was this ugly!"
	}
}
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("mokou is studying this map.", "mokouHUD",
		ScrW() / 2, ScrH() / 2, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "mokouHUD",
		ScrW() / 2, ScrH() / 2, color_white,
	...
	draw.SimpleTextOutlined("Time Elapsed:", "mokouHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "mokouHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
	...
	draw.SimpleTextOutlined(flavourText, "mokouHUDSmall",
		ScrW() / 2, ScrH() * 4/5, color_yellow,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("mokou_navgen", function()
	local startSuccess = net.ReadBool()
	if startSuccess then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "mokouNavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message("Oh no. mokou doesn't even know where to start with \z
		this map.\n\z
...
local function requestNavGenerate()
	RunConsoleCommand("npc_mokou_learn")
end
...
local function navWarning()
	Derma_Query("It will take a while (possibly hours) for mokou to figure \z
		this map out.\n\z
...
net.Receive("mokou_nag", function()
	if not nagMe then return end

	if game.SinglePlayer() then
		Derma_Query("Uh oh! mokou doesn't know this map.\n\z
			Would you like him to learn it?",
			"This map is not yet mokou-compatible!",
			"Yes", navWarning,
			"No", nil,
			"No. Don't ask again.", stopNagging)
	else
		Derma_Query("Uh oh! mokou doesn't know this map. \z
			He won't be able to move!\n\z
			Because you're not in a single-player game, he isn't able to \z
			learn it.\n\z
			\n\z
			Ask the server host about teaching this map to mokou.\n\z
			\n\z
			If you ARE the server host, you can run npc_mokou_learn over \z
			rcon.\n\z
			Keep in mind that it may take hours during which you will be \z
			unable\n\z
			to play, and THE MAP WILL BE RESTARTED.",
			"This map is currently not mokou-compatible!",
			"Ok", nil,
