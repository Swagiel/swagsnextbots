...
ENT.JumpSound = Sound("npc_nardwuar/jump.mp3");
ENT.JumpHighSound = Sound("npc_nardwuar/spring.mp3");

ENT.TauntSounds = {
	Sound("npc_nardwuar/youare.mp3");
};

local chaseMusic = Sound("npc_nardwuar/theme.mp3");
local IsValid = IsValid;

if (SERVER) then -- SERVER --

local npc_nardwuar_acquire_distance =
	CreateConVar("npc_nardwuar_acquire_distance", 2500, FCVAR_NONE,
	"The maximum distance at which nardwuar will chase a target.")

local npc_nardwuar_spawn_protect =
	CreateConVar("npc_nardwuar_spawn_protect", 1, FCVAR_NONE,
	"If set to 1, nardwuar will not target players or hide within 200 units of \z
	a spawn point.")

local npc_nardwuar_attack_distance =
	CreateConVar("npc_nardwuar_attack_distance", 80, FCVAR_NONE,
	"The reach of nardwuar's attack.")

local npc_nardwuar_attack_interval =
	CreateConVar("npc_nardwuar_attack_interval", 0.2, FCVAR_NONE,
	"The delay between nardwuar's attacks.")

local npc_nardwuar_attack_force =
	CreateConVar("npc_nardwuar_attack_force", 800, FCVAR_NONE,
	"The physical force of nardwuar's attack. Higher values throw things \z
	farther.")

local npc_nardwuar_smash_props =
	CreateConVar("npc_nardwuar_smash_props", 1, FCVAR_NONE,
	"If set to 1, nardwuar will punch through any props placed in their way.")

local npc_nardwuar_allow_jump =
	CreateConVar("npc_nardwuar_allow_jump", 1, FCVAR_NONE,
	"If set to 1, nardwuar will be able to jump.")

local npc_nardwuar_hiding_scan_interval =
	CreateConVar("npc_nardwuar_hiding_scan_interval", 3, FCVAR_NONE,
	"nardwuar will only seek out hiding places every X seconds. This can be an \z
	expensive operation, so it is not recommended to lower this too much. \z
	However, if distant nardwuars are not hiding from you quickly enough, you \z
	may consider lowering this a small amount.")

local npc_nardwuar_hiding_repath_interval =
	CreateConVar("npc_nardwuar_hiding_repath_interval", 1, FCVAR_NONE,
	"The path to nardwuar's hiding spot will be redetermined every X seconds.")

local npc_nardwuar_chase_repath_interval =
	CreateConVar("npc_nardwuar_chase_repath_interval", 0.1, FCVAR_NONE,
	"The path to and position of nardwuar's target will be redetermined every \z
	X seconds.")

local npc_nardwuar_expensive_scan_interval =
	CreateConVar("npc_nardwuar_expensive_scan_interval", 1, FCVAR_NONE,
	"Slightly expensive operations (distance calculations and entity \z
	searching) will occur every X seconds.")

local npc_nardwuar_force_download =
	CreateConVar("npc_nardwuar_force_download", 1, FCVAR_ARCHIVE,
	"If set to 1, clients will be forced to download nardwuar resources \z
...
util.AddNetworkString("nardwuar_nag")
util.AddNetworkString("nardwuar_navgen")
...
local VECTOR_nardwuar_HEIGHT = Vector(0, 0, 96)
local function isPointSuitableForHiding(point)
	trace.start = point
	trace.endpos = point + VECTOR_nardwuar_HEIGHT
...
local function buildHidingSpotCache()
	...
	print(string.format("npc_nardwuar: found %d suitable (%d unsuitable) hiding \z
...
local function isValidTarget(ent)
	...
	-- Ignore dead NPCs, other nardwuars, and dummy NPCs.
	local class = ent:GetClass()
	return (ent:IsNPC()
		and ent:Health() > 0
		and class ~= "npc_nardwuar"
		and not class:find("bullseye"))
end

hook.Add("PlayerSpawnedNPC", "nardwuarMissingNavmeshNag", function(ply, ent)
	if not IsValid(ent) then return end
	if ent:GetClass() ~= "npc_nardwuar" then return end
	if navmesh.GetNavAreaCount() > 0 then return end

	-- Try to explain why nardwuar isn't working.
	net.Start("nardwuar_nag")
	net.Send(ply)
end)
...
local function navEndGenerate()
	...
	if not navmesh.IsGenerating() then
		print("npc_nardwuar: Navmesh generation completed in " .. timeElapsedStr)
	else
		print("npc_nardwuar: Navmesh generation aborted after " .. timeElapsedStr)
	end
...
local function navGenerate()
	...
	if next(seeds, nil) == nil then
		print("npc_nardwuar: Couldn't find any places to seed nav_generate")
		return false
	end
	...
	for seed in pairs(seeds) do
		...
		if not tr.StartSolid and tr.Hit then
			print(string.format("npc_nardwuar: Adding seed %s at %s", seed, pos))
			navmesh.AddWalkableSeed(tr.HitPos, tr.HitNormal)
		else
			print(string.format("npc_nardwuar: Couldn't add seed %s at %s", seed,
				pos))
		end
	end

	-- The least we can do is ensure they don't have to listen to this noise.
	for _, nardwuar in pairs(ents.FindByClass("npc_nardwuar")) do
		nardwuar:Remove()
	end
	...
	if navmesh.IsGenerating() then
		...
		hook.Add("ShutDown", "nardwuarNavGen", navEndGenerate)
	else
		print("npc_nardwuar: nav_generate failed to initialize")
...
concommand.Add("npc_nardwuar_learn", function(ply, cmd, args)
	if navmesh.IsGenerating() then
		return
	end
	...
	if game.SinglePlayer() then
		print("npc_nardwuar: Beginning nav_generate requested by " .. ply:Name())

		-- Disable expensive computations in single-player. nardwuar doesn't use
	...
	elseif isConsole then
		print("npc_nardwuar: Beginning nav_generate requested by server console")
	else
		return
	end
...
function ENT:GetNearestTarget()
	...
	local maxAcquireDist = npc_nardwuar_acquire_distance:GetInt()
	...
	for _, ent in pairs(acquirableEntities) do
		...
		-- if `npc_nardwuar_spawn_protect' = 1.
		--TODO: Only for the first few seconds?
		if npc_nardwuar_spawn_protect:GetBool() and ent:IsPlayer()
...
function ENT:AttackNearbyTargets(radius)
	local attackForce = npc_nardwuar_attack_force:GetInt()
	...
	for _, ent in pairs(nearEntities) do
		if isValidTarget(ent) then
			...
			-- Give the player a good whack. nardwuar means business.
		...
		elseif ent:GetMoveType() == MOVETYPE_VPHYSICS then
			if not npc_nardwuar_smash_props:GetBool() then continue end
...
function ENT:AttemptJumpAtTarget()
	...
	local maxAttackDistance = npc_nardwuar_attack_distance:GetInt()
...
function ENT:BehaveUpdate() --TODO: Split this up more. Eww.
	...
	local scanInterval = npc_nardwuar_expensive_scan_interval:GetFloat()
	...
	if IsValid(self.CurrentTarget) then
		...
		local attackInterval = npc_nardwuar_attack_interval:GetFloat()
		if currentTime - self.LastAttack > attackInterval then
			local attackDistance = npc_nardwuar_attack_distance:GetInt()
		...
		local repathInterval = npc_nardwuar_chase_repath_interval:GetFloat()
		...
		if self:IsOnGround() and npc_nardwuar_allow_jump:GetBool()
		...
		local hidingScanInterval = npc_nardwuar_hiding_scan_interval:GetFloat()
		...
		if self.HidingSpot ~= nil then
			local hidingInterval = npc_nardwuar_hiding_repath_interval:GetFloat()
...
local MAT_nardwuar = Material("npc_nardwuar/nardwuar")
killicon.Add("npc_nardwuar", "npc_nardwuar/killicon", color_white)
language.Add("npc_nardwuar", "nardwuar ")
...
local function DevPrint(devLevel, msg)
	if developer:GetInt() >= devLevel then
		print("npc_nardwuar: " .. msg)
	end
end

local panicMusic = nil
local lastPanic = 0 -- The last time we were in music range of a nardwuar.

--TODO: Why don't these flags show up? Bug? Documentation would be lovely.
local npc_nardwuar_music_volume =
	CreateConVar("npc_nardwuar_music_volume", 1,
	bit.bor(FCVAR_DEMO, FCVAR_ARCHIVE),
	"Maximum music volume when being chased by nardwuar. (0-1, where 0 is muted)")

-- If another nardwuar comes in range before this delay is up,
-- the music will continue where it left off.
local MUSIC_RESTART_DELAY = 2

-- Beyond this distance, nardwuars do not count to music volume.
local MUSIC_CUTOFF_DISTANCE = 1000

-- Max volume is achieved when MUSIC_nardwuar_PANIC_COUNT nardwuars are this close,
-- or an equivalent score.
local MUSIC_PANIC_DISTANCE = 200

 -- That's a lot of nardwuar.
local MUSIC_nardwuar_PANIC_COUNT = 8

local MUSIC_nardwuar_MAX_DISTANCE_SCORE =
	(MUSIC_CUTOFF_DISTANCE - MUSIC_PANIC_DISTANCE) * MUSIC_nardwuar_PANIC_COUNT

local function updatePanicMusic()
	if #ents.FindByClass("npc_nardwuar") == 0 then
		-- Whoops. No need to run for now.
		DevPrint(4, "Halting music timer.")
		timer.Remove("nardwuarPanicMusicUpdate")
	...
	local userVolume = math.Clamp(npc_nardwuar_music_volume:GetFloat(), 0, 1)
	...
	for _, ent in pairs(nearEntities) do
		if IsValid(ent) and ent:GetClass() == "npc_nardwuar" then
	...
	local musicVolume = math.min(1,
		totalDistanceScore / MUSIC_nardwuar_MAX_DISTANCE_SCORE)
	...
	if musicVolume > 0 then
		...
		if not LocalPlayer():Alive() then
			-- Quiet down so we can hear nardwuar taunt us.
...
local function startTimer()
	if not timer.Exists("nardwuarPanicMusicUpdate") then
		timer.Create("nardwuarPanicMusicUpdate", 0.05, REPEAT_FOREVER,
			updatePanicMusic)
...
function ENT:DrawTranslucent()
	render.SetMaterial(MAT_nardwuar)

	-- Get the normal vector from nardwuar to the player's eyes, and then compute
	...
	-- nardwuar should only look 1/3 of the way up to the player so that they
...
surface.CreateFont("nardwuarHUD", {
	font = "Arial",
	size = 56
})

surface.CreateFont("nardwuarHUDSmall", {
	font = "Arial",
	size = 24
})
...
local function navGenerateHUDOverlay()
	draw.SimpleTextOutlined("nardwuar is studying this map.", "nardwuarHUD",
		ScrW() / 2, ScrH() / 2, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_TOP, 2, color_black)
	draw.SimpleTextOutlined("Please wait...", "nardwuarHUD",
		ScrW() / 2, ScrH() / 2, color_white,
	...
	draw.SimpleTextOutlined("Time Elapsed:", "nardwuarHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_BOTTOM, 1, color_black)
	draw.SimpleTextOutlined(elapsedStr, "nardwuarHUDSmall",
		ScrW() / 2, ScrH() * 3/4, color_white,
	...
	draw.SimpleTextOutlined(flavourText, "nardwuarHUDSmall",
		ScrW() / 2, ScrH() * 4/5, color_yellow,
		TEXT_ALIGN_CENTER, TEXT_ALIGN_CENTER, 1, color_black)
end

net.Receive("nardwuar_navgen", function()
	local startSuccess = net.ReadBool()
	if startSuccess then
		generateStart = SysTime()
		lastBracket = 0
		hook.Add("HUDPaint", "nardwuarNavGenOverlay", navGenerateHUDOverlay)
	else
		Derma_Message("Oh no. nardwuar doesn't even know where to start with \z
		this map.\n\z
...
local function requestNavGenerate()
	RunConsoleCommand("npc_nardwuar_learn")
end
...
local function navWarning()
	Derma_Query("It will take a while (possibly hours) for nardwuar to figure \z
		this map out.\n\z
...
net.Receive("nardwuar_nag", function()
	if not nagMe then return end

	if game.SinglePlayer() then
		Derma_Query("Uh oh! nardwuar doesn't know this map.\n\z
			Would you like him to learn it?",
			"This map is not yet nardwuar-compatible!",
			"Yes", navWarning,
			"No", nil,
			"No. Don't ask again.", stopNagging)
	else
		Derma_Query("Uh oh! nardwuar doesn't know this map. \z
			He won't be able to move!\n\z
			Because you're not in a single-player game, he isn't able to \z
			learn it.\n\z
			\n\z
			Ask the server host about teaching this map to nardwuar.\n\z
			\n\z
			If you ARE the server host, you can run npc_nardwuar_learn over \z
			rcon.\n\z
			Keep in mind that it may take hours during which you will be \z
			unable\n\z
			to play, and THE MAP WILL BE RESTARTED.",
			"This map is currently not nardwuar-compatible!",
			"Ok", nil,
